"use client";
// PCalendar — 1:1 port of provider-app.jsx PCalendar (drag-and-drop scheduling).
...
const TODAY = 10;
const PCAL_TIMES = ["07:30", "09:00", "11:00", "13:00", "15:00", "17:00", "18:30", "20:00"];

type Svc = any; type Slot = any;

export function PCalendar({ svcs, slots }: { svcs: Svc[]; slots: Slot[] }) {
  const router = useRouter();
  const days = Array.from({ length: 7 }, (_, i) => TODAY + i).filter((d) => d <= 30);
  ...
  const nextTime = (arr: Slot[]) => { const used = new Set(arr.map((s) => s.time)); return PCAL_TIMES.find((tm) => !used.has(tm)) || "12:00"; };

  const startSvc = (sid: string, e: React.DragEvent) => { drag.current = { kind: "service", sid }; setDragId("svc:" + sid); e.dataTransfer.effectAllowed = "copy"; try { e.dataTransfer.setData("text/plain", sid); } catch {} };
  const startSlot = (slot: Slot, e: React.DragEvent) => { e.stopPropagation(); drag.current = { kind: "slot", slotId: slot.id, fromDay: slot.day, time: slot.time }; setDragId("slot:" + slot.id); e.dataTransfer.effectAllowed = "move"; try { e.dataTransfer.setData("text/plain", slot.id); } catch {} };
  const endDrag = () => { drag.current = null; setDragId(null); setOver(null); };
  const drop = (day: number, e: React.DragEvent) => {
    e.preventDefault(); const p = drag.current; setOver(null);
    if (!p) return;
    if (p.kind === "service") rpc("addSlot", { serviceId: p.sid, day, time: nextTime(byDay[day] || []) }).then(() => router.refresh());
    else if (p.kind === "slot" && p.fromDay !== day) { const used = new Set((byDay[day] || []).map((s) => s.time)); rpc("moveSlot", { slotId: p.slotId, day, time: used.has(p.time) ? nextTime(byDay[day] || []) : p.time }).then(() => router.refresh()); }
    endDrag();
  };
  const remove = (id: string) => rpc("removeSlot", { slotId: id }).then(() => router.refresh());
  const setTime = (id: string, time: string) => { setEditing(null); rpc("setSlotTime", { slotId: id, time }).then(() => router.refresh()); };
    ...
    <div className="anim-fade">
      ...
      <div className="pcal-wrap">
        ...
        <div className="pcal-week">
          {days.map((d) => {
            const list = byDay[d] || []; const today = d === TODAY; const isOver = over === d;
            return (
              <div key={d} className={`pcal-col ${today ? "today" : ""} ${isOver ? "over" : ""}`}
                onDragOver={(e) => { e.preventDefault(); if (over !== d) setOver(d); }}
                onDragLeave={(e) => { if (!e.currentTarget.contains(e.relatedTarget as Node)) setOver((o) => (o === d ? null : o)); }}
                onDrop={(e) => drop(d, e)}>
                <div className={`pcal-col-h ${today ? "today" : ""}`}>
                  <div className="wd">{WD[dow(d)]}</div><div className="dn">{d}</div>
                  {list.length > 0 && <span className="pcal-cnt">{list.length}</span>}
                </div>
                <div className="pcal-col-b">
                  {list.length === 0
                    ? <div className="pcal-drop">{isOver ? "Release to schedule" : "Drop a service here"}</div>
                    : list.map((slot) => { const svc = svcs.find((x) => x.id === slot.serviceId) || { name: "Service", mood: "calm", cap: 0 }; const m = MOODS[svc.mood]; return (
