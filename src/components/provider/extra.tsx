    ...
    <div className="anim-fade max-w-[840px]">
      <div className="card" style={{ overflow: "hidden", marginBottom: "var(--gap)" }}>
        ...
        <div className="pt-0 px-[24px] pb-[22px] flex gap-[18px] items-end mt-[-36px] flex-wrap relative z-[1]">
          <div className="w-[84px] h-[84px] rounded-[22px] bg-[linear-gradient(140deg,var(--m-calm),#2E8C80)] grid text-[#fff] font-extrabold text-[34px] flex-none" style={{ border: "4px solid var(--surface)", placeItems: "center", fontFamily: "var(--display)" }}>{(f.name || "?")[0]}</div>
          <div className="flex-1 min-w-[200px] pb-[4px]">
            <h2 className="text-[24px]">{f.name}</h2>
            <div className="text-ink-3 font-semibold mt-[3px]">{edit ? <Input placeholder="A short tagline customers see" value={f.tagline} onChange={(e) => set("tagline", e.target.value)} style={{ marginTop: 6 }} /> : (f.tagline || "Add a tagline so customers know your vibe")}</div>
          </div>
      ...
      <div className="grid grid-cols-[1.4fr_1fr] gap-[var(--gap)] items-start">
        <div className="card" style={{ padding: 24 }}>
          ...
          {edit ? <Textarea rows={5} value={f.about} onChange={(e) => set("about", e.target.value)} placeholder="Tell customers your story…" style={{ resize: "vertical", lineHeight: 1.5 }} /> : <p className="text-ink-2 text-[14.5px] leading-[1.6] m-0">{f.about || "No description yet — hit Edit and tell customers what makes you special."}</p>}
          ...
          <div className="grid grid-cols-[1fr_1fr] gap-[16px]">
            {([["email", "Email", "mail"], ["phone", "Phone", "phone"], ["site", "Website", "compass"], ["address", "Address", "pin"]] as const).map(([k, l, ic]) => {
              ...
              return <div key={k} style={{ gridColumn: k === "address" ? "1 / -1" : "auto" }}>
                <div className="text-[12px] font-bold text-ink-3 mb-[6px]">{l}</div>
                {edit ? <Input value={(f as any)[k]} onChange={(e) => set(k, e.target.value)} /> : <div className="flex gap-[8px] items-center font-semibold text-[14px]"><span className="text-ink-3"><I size={16} /></span>{(f as any)[k] || "—"}</div>}
              </div>;
            })}
          </div>
        </div>
        ...
        <div className="flex flex-col gap-[var(--gap)]">
          <div className="card" style={{ padding: 22 }}>
            ...
            <div className="flex justify-between text-[13.5px]">
              <div><div className="text-ink-3 font-semibold mb-[4px]">Founded</div><b>{f.founded}</b></div>
              <div><div className="text-ink-3 font-semibold mb-[4px]">Team</div><b>{f.team} people</b></div>
              <div><div className="text-ink-3 font-semibold mb-[4px]">Joined</div><b>{p.joined || "Jun 2026"}</b></div>
            </div>
          </div>
          <div className="card" style={{ padding: 22 }}>
            <div className="flex items-center justify-between mb-[6px]">
              <h3 className="text-[16px]">Gallery</h3><span className="text-[12.5px] text-ink-3 font-semibold">{P_GALLERY.length} items</span>
            </div>
            <div className="grid grid-cols-[repeat(3,1fr)] gap-[8px] mt-[10px]">
              {P_GALLERY.slice(0, 6).map((g) => <div key={g.id} className="rounded-xs" style={{ aspectRatio: "1", background: g.g }} />)}
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="anim-fade max-w-[820px]">
      <div className="card" style={{ padding: 22, marginBottom: "var(--gap)" }}>
        <div className="shead" style={{ marginBottom: 14 }}><div><h3 className="text-[17px]">Base prices</h3><div className="text-[13px] text-ink-3 font-semibold">Per person, before dynamic rules</div></div></div>
        {svcs.length === 0
          ? <div className="py-[20px] px-0 text-ink-3 font-semibold text-[13.5px]">No services yet — create one in Services.</div>
          : <table className="tbl"><thead><tr><th>Service</th><th>Duration</th><th>Capacity</th><th>Base price</th></tr></thead><tbody>
            {svcs.map((s) => <tr key={s.id} className="row"><td><b className="font-bold">{s.name}</b></td><td className="text-ink-2">{s.dur}</td><td className="text-ink-2">{s.cap}</td><td className="font-bold" style={{ fontFamily: "var(--display)" }}>{money(s.price)}</td></tr>)}
          </tbody></table>}
      </div>
      <div className="shead"><div><h3 className="text-[17px]">Dynamic pricing rules</h3><div className="text-[13px] text-ink-3 font-semibold">Automatically adjust prices to fill capacity</div></div><Btn size="md" icon={<Icons.plus size={16} />}>New rule</Btn></div>
      <div className="flex flex-col gap-[12px]">
        {rules.map((r) => <div key={r.id} className="card" style={{ padding: "16px 20px", display: "flex", alignItems: "center", gap: 16, opacity: r.active ? 1 : 0.6 }}>
          <span className="w-[42px] h-[42px] rounded-sm flex-none grid" style={{ placeItems: "center", background: r.type === "up" ? "rgba(224,33,47,.12)" : "rgba(31,164,110,.13)", color: r.type === "up" ? "var(--coral)" : "#1FA46E" }}><Icons.percent size={20} /></span>
          <div className="flex-1 min-w-0"><div className="font-bold text-[15px]">{r.name}</div><div className="text-[13px] text-ink-3 font-semibold">{r.cond}</div></div>
          <span className="font-extrabold text-[18px]" style={{ fontFamily: "var(--display)", color: r.type === "up" ? "var(--coral)" : "#1FA46E" }}>{r.adj}</span>
...
export function PGalleryView() {
  ...
  return (
    <div className="anim-fade">
      <div className="shead"><div><div className="eyebrow" style={{ marginBottom: 6 }}>{items.length} items</div><h2 className="text-[22px]">Photos & videos</h2></div><Btn size="md" icon={<Icons.plus size={16} />}>Upload</Btn></div>
      <div className="grid grid-cols-[repeat(auto-fill,minmax(220px,1fr))] gap-[var(--gap)]">
        <label className="rounded-lg flex flex-col items-center justify-center gap-[8px] text-ink-3 cursor-pointer bg-surface-2" style={{ aspectRatio: "4/3", border: "2px dashed var(--line-2)" }}><Icons.image size={28} /><span className="font-bold text-[13.5px]">Add photo or video</span></label>
        {items.map((g) => <div key={g.id} className="card anim-pop" style={{ overflow: "hidden", padding: 0 }}>
          <div className="relative" style={{ aspectRatio: "4/3", background: g.g }}>
            ...
            {g.video && <span className="absolute inset-0 grid" style={{ placeItems: "center" }}><span className="w-[46px] h-[46px] rounded-[99px] bg-[rgba(0,0,0,.45)] grid text-[#fff]" style={{ placeItems: "center" }}><Icons.arrowR size={20} /></span></span>}
            <div className="absolute bottom-0 left-0 right-0 pt-[24px] px-[12px] pb-[10px] bg-[linear-gradient(transparent,rgba(0,0,0,.5))] flex items-center gap-[6px]"><span className="text-[#fff] font-bold text-[12.5px] flex-1 whitespace-nowrap overflow-hidden" style={{ textOverflow: "ellipsis" }}>{g.label}</span></div>
          </div>
...
export function PMarketing() {
  ...
  return (
    <div className="anim-fade max-w-[860px]">
      <div className="shead"><div><h3 className="text-[17px]">Promo codes</h3><div className="text-[13px] text-ink-3 font-semibold">Drive bookings with limited-time offers</div></div><Btn size="md" icon={<Icons.plus size={16} />} onClick={() => setModal(true)}>Create code</Btn></div>
      <div className="grid grid-cols-[repeat(auto-fill,minmax(260px,1fr))] gap-[var(--gap)]">
        {promos.map((p, i) => <div key={i} className="card anim-pop" style={{ padding: 20 }}>
          <div className="flex items-center justify-between mb-[12px]">
            <span className="font-extrabold text-[18px] tracking-[.04em] py-[5px] px-[12px] rounded-xs bg-[color-mix(in_srgb,var(--orange)_14%,transparent)] text-[var(--orange-deep)]" style={{ fontFamily: "var(--display)", border: "1px dashed color-mix(in srgb,var(--orange) 45%,transparent)" }}>{p.code}</span>
            <Pill status={p.status} label={p.status === "active" ? "Active" : p.status === "rejected" ? "Expired" : p.status} />
          </div>
          <p className="mt-0 mx-0 mb-[14px] text-[14px] text-ink-2 font-semibold">{p.desc}</p>
          <div className="h-[7px] rounded-[99px] bg-surface-2 overflow-hidden mb-[8px]"><div className="h-full rounded-[99px]" style={{ width: `${Math.min(p.uses / p.cap * 100, 100)}%`, background: p.uses >= p.cap ? "var(--ink-3)" : "var(--coral)" }} /></div>
          <div className="flex justify-between text-[12.5px] text-ink-3 font-semibold"><span>{p.uses}/{p.cap} redeemed</span><span>Expires {p.expires}</span></div>
        </div>)}
      </div>
    ...
    <Modal onClose={onClose} maxWidth={460}>
      <div className="py-[24px] px-[26px]">
        ...
        <div className="flex flex-col gap-[14px]">
          <div><div className="text-[12.5px] font-bold text-ink-2 mb-[7px]">Code</div><Input placeholder="SUMMER20" value={code} onChange={(e) => setCode(e.target.value.toUpperCase())} style={{ fontFamily: "var(--display)", fontWeight: 700, letterSpacing: ".05em" }} /></div>
          <div><div className="text-[12.5px] font-bold text-ink-2 mb-[7px]">Description</div><Input placeholder="15% off any wellness session" value={desc} onChange={(e) => setDesc(e.target.value)} /></div>
          <div className="flex gap-[12px]">
            <div className="flex-1"><div className="text-[12.5px] font-bold text-ink-2 mb-[7px]">Discount</div><div className="flex gap-[8px]"><Input value={amt} onChange={(e) => setAmt(e.target.value)} style={{ flex: 1 }} /><Seg value={unit} options={[{ v: "%", l: "%" }, { v: "₽", l: "₽" }]} onChange={setUnit} /></div></div>
            <div className="w-[110px]"><div className="text-[12.5px] font-bold text-ink-2 mb-[7px]">Max uses</div><Input value={cap} onChange={(e) => setCap(e.target.value)} /></div>
          </div>
        </div>
