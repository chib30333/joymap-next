    ...
    <div className="anim-fade">
      <div className="flex items-end justify-between gap-[20px] flex-wrap mt-[6px] mb-[22px]">
        <div>
          ...
          <h1 className="max-w-[560px] leading-[1.02]" style={{ fontSize: "clamp(30px,4vw,44px)" }}>
            {greet}, {userName}. <span className="text-[var(--orange)]">Here&apos;s your week of joy.</span>
          </h1>
      ...
      {catalog.length === 0 ? (
        ...
        <div className="card" style={{ padding: "56px 24px", textAlign: "center" }}>
          <div className="w-[60px] h-[60px] rounded-[99px] bg-coral-soft text-coral-deep grid mt-0 mb-[14px] mx-auto" style={{ placeItems: "center" }}><Icons.sparkle size={26} /></div>
          <h3 className="text-[20px]">No Joy Map yet</h3>
          <p className="text-ink-2 text-[14.5px] mt-[8px] mb-[18px] mx-auto max-w-[380px]">Let Joy compose a week of experiences around your moods.</p>
        ...
        <>
          ...
          {days.length > 0 && (
            <div className="card-fill" style={{ marginTop: 26, padding: "22px 24px", display: "flex", gap: 18, alignItems: "flex-start" }}>
              ...
              <div>
                <h3 className="text-[17px] mb-[6px]">Why Joy built this week</h3>
                <p className="text-[rgba(243,235,224,.85)] text-[14.5px] leading-[1.55] max-w-[760px]">
                  You asked for more <b className="text-[#FFF3E8]">{(userMoods || []).map((k) => MOODS[k] ? MOODS[k].label.toLowerCase() : k).join(", ") || "joy"}</b>. The week balances restorative mornings with playful evenings — and keeps a true rest day so the week breathes.
                </p>
              </div>
            </div>
          )}
...
export function EmptyMarketplace() {
  return (
    <div className="card" style={{ padding: "60px 24px", textAlign: "center" }}>
      <div className="w-[64px] h-[64px] rounded-[99px] bg-surface-2 grid mt-0 mb-[16px] mx-auto text-ink-3" style={{ placeItems: "center" }}><Icons.compass size={30} /></div>
...
function MoodArc({ map, byId }: { map: Day[]; byId: (id: string | null) => Exp | null }) {
  return (
    <div className="card" style={{ padding: "14px 18px", display: "flex", alignItems: "center", gap: 14, overflowX: "auto" }}>
      <span className="text-[12.5px] font-bold text-ink-3 tracking-[.04em] uppercase flex-none">Your emotional arc</span>
        ...
        {map.map((d) => {
          ...
          return (
            <div key={d.day} className="text-center flex-1">
              <div className="h-[8px] rounded-[99px] mb-[6px]" style={{ background: m ? m.color : "var(--line-2)", boxShadow: m ? `0 2px 8px ${m.color}55` : "none" }} />
    ...
    return (
      <div className="card anim-pop" style={{ flex: "none", width: 230, scrollSnapAlign: "start", padding: 20, display: "flex", flexDirection: "column", background: "var(--surface-2)", borderStyle: "dashed", animationDelay: `${i * 0.06}s` }}>
        <div className="flex items-center justify-between">
          <span className="font-display font-extrabold text-[15px]">{d.wd} · {d.date}</span>
          {today && <span className="bg-coral text-[#fff] pt-[3px] pb-[3px] pl-[9px] pr-[9px] rounded-[99px] text-[10.5px] font-extrabold">TODAY</span>}
        </div>
        <div className="flex-1 flex flex-col justify-center items-center text-center gap-[10px] pt-[18px] pb-[18px] pl-0 pr-0">
          <div className="w-[48px] h-[48px] rounded-[99px] bg-[var(--m-calm-soft)] grid text-[var(--m-calm)]" style={{ placeItems: "center" }}><Icons.heart size={24} /></div>
          <div className="font-display font-bold text-[17px] whitespace-nowrap">Rest day</div>
    ...
    <div className="card anim-pop" onClick={() => onOpen(e)} style={{ flex: "none", width: 268, scrollSnapAlign: "start", overflow: "hidden", cursor: "pointer", transition: ".2s", animationDelay: `${i * 0.06}s` }}>
      <PhotoFrame exp={e} ratio="16/11">
        <div className="absolute top-[12px] left-[12px] right-[12px] flex justify-between items-start">
          <span className="bg-[rgba(255,255,255,.92)] text-[#241C2E] pt-[5px] pb-[5px] pl-[11px] pr-[11px] rounded-[99px] text-[12px] font-extrabold">{d.wd} · {d.date}</span>
          {today && <span className="bg-coral text-[#fff] pt-[5px] pb-[5px] pl-[10px] pr-[10px] rounded-[99px] text-[11px] font-extrabold">TODAY</span>}
        </div>
      ...
      <div className="pt-[13px] pr-[15px] pb-[15px] pl-[15px] flex flex-col gap-[11px]">
        <div className="flex items-center justify-between">
          ...
          <span className="inline-flex items-center gap-[5px] text-[13px] font-bold text-ink-2"><Icons.clock size={14} />{d.time}</span>
        </div>
