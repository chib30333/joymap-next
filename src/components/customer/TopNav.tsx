  ...
  return (
    <header className="topnav">
      <div className="tn-row1">
        ...
        <div className="flex-1 max-w-[460px] relative" style={{ marginInline: 8 }}>
          <span className="absolute text-ink-3" style={{ insetInlineStart: 15, top: "50%", transform: "translateY(-50%)" }}><Icons.search size={18} /></span>
          <Input placeholder={t("Search activities, moods, places…")} value={query}
        ...
        <div className="menu-pop anim-pop" style={{ minWidth: 180 }}>
          {CITIES.map((c) => (
            <button key={c} className={`menu-item ${c === city ? "on" : ""}`} onClick={() => pick(c)}>
              <Icons.pin size={16} />{c}{c === city && <span className="text-coral" style={{ marginInlineStart: "auto" }}><Icons.check size={16} /></span>}
            </button>
          ))}
        </div>
      )}
        ...
        <div className="menu-pop anim-pop">
          <div className="flex items-center gap-[11px] pt-[8px] px-[10px] pb-[12px]">
            ...
            <div className="min-w-0">
              <div className="font-extrabold font-display text-[15px]">{user.name}</div>
              <div className="text-[12px] text-[var(--orange)] font-bold inline-flex items-center gap-[4px]"><Icons.sparkle size={12} />{user.plan}</div>
            </div>
          </div>
          ...
          <a className="menu-item on" style={{ cursor: "default" }}><span className="w-[7px] h-[7px] rounded-[99px] bg-coral" />{t("Customer")}<span className="font-bold text-[11px]" style={{ marginInlineStart: "auto" }}>{t("You")}</span></a>
          <Link className="menu-item" href="/provider"><span className="w-[7px] h-[7px] rounded-[99px] bg-[var(--ink-3)]" />{t("Provider")}</Link>
