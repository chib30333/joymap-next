        ...
        {list.map((n) => {
          ...
          return (
            <div key={n.id} className="card" onClick={() => mark(n.id)} style={{ padding: "15px 17px", display: "flex", gap: 13, cursor: "pointer", borderColor: n.unread ? "color-mix(in srgb,var(--coral) 32%,transparent)" : "var(--line)" }}>
              <span className="w-10 h-10 rounded-sm flex-none grid place-items-center" style={{ background: `color-mix(in srgb,${n.accent} 15%,transparent)`, color: n.accent }}><I size={19} /></span>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <span className="font-bold text-[14.5px]">{n.title}</span>
                  {n.unread && <span className="w-2 h-2 rounded-[99px] bg-coral flex-none" />}
                  <span className="ml-auto text-[12px] text-ink-3 font-semibold flex-none">{n.time}</span>
                </div>
                <p className="mt-[3px] mx-0 mb-0 text-[13.5px] text-ink-2 leading-[1.45]">{n.body}</p>
              </div>
            </div>
          );
        })}
        {list.length === 0 && <div className="text-center p-[50px] text-ink-3"><Icons.checkCirc size={36} /><p className="mt-2.5 font-semibold">You&apos;re all caught up.</p></div>}
      </div>
    </div>
  );
}
