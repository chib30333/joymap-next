    ...
    <div className="anim-fade corp">
      <div className="corp-hero">
        <div className="relative flex gap-[30px] items-center flex-wrap">
          <div className="flex-1 min-w-[280px] max-w-[560px]">
            ...
            <h1 className="text-[#FFF3E4] leading-[1.05]" style={{ fontSize: "clamp(28px,3.4vw,40px)", maxWidth: "14ch" }}>Your wellbeing, on the house.</h1>
            <p className="text-[rgba(244,233,216,.8)] text-[15.5px] leading-[1.6] mt-[14px]" style={{ maxWidth: "52ch" }}>
          ...
          <div className="flex flex-col items-center gap-[14px]">
            <div className="ring" style={{ ["--p" as any]: pct }}>
              <div className="hole">
                <div>
                  <div className="font-display font-extrabold text-[24px] text-[#FFF3E4] leading-[1]">{fmt(remaining)}</div>
                  <div className="text-[10.5px] text-[rgba(244,233,216,.7)] font-bold tracking-[.04em] mt-[3px]">LEFT THIS MONTH</div>
                </div>
              </div>
            </div>
            <div className="text-center text-[12.5px] text-[rgba(244,233,216,.7)] font-semibold">{fmt(COMPANY.used)} of {fmt(COMPANY.allowance)} used<br />Renews {COMPANY.renews}</div>
          </div>
        </div>
      </div>

      <div className="mt-[34px] mx-0 mb-[14px]"><SectionHead eyebrow="Included with your plan" title="Your perks" /></div>
        ...
        {CORP_PERKS.map((p) => {
          ...
          return (
            <div key={p.title} className="corp-card corp-perk">
              ...
              <p className="text-[13px] text-ink-3 font-semibold mt-0 mx-0 mb-[14px] leading-[1.45]">{p.desc}</p>
              <div className="inline-flex items-center gap-[6px] font-extrabold font-display text-[15px]" style={{ color: p.accent }}><span style={{ width: 7, height: 7, borderRadius: 99, background: p.accent }} />{p.value}</div>
            </div>
          );
        })}
      </div>
      ...
      <div className="grid grid-cols-[1.3fr_1fr] gap-[var(--gap)] mt-[40px] mx-0 mb-0 items-stretch">
        <div className="corp-gift" onClick={() => setModal("gift")}>
          <div className="relative flex flex-col h-full">
            ...
            <h3 className="text-[24px] text-[#FFF3E4] mb-[8px]" style={{ maxWidth: "16ch" }}>Share the joy with a colleague.</h3>
            <p className="text-[rgba(244,233,216,.78)] text-[14px] leading-[1.55] mb-[20px]" style={{ maxWidth: "46ch" }}>Send an experience gift card by email — they pick what brings them joy.</p>
            <div className="flex gap-[8px] mt-auto flex-wrap">
              {GIFT_DENOMS.map((d) => <span key={d} className="py-[7px] px-[13px] rounded-[99px] border border-[color-mix(in_srgb,var(--gold)_45%,transparent)] font-extrabold text-[13px] font-display">{fmt(d)}</span>)}
            </div>
          </div>
        </div>
        <div className="corp-card" style={{ display: "flex", flexDirection: "column", justifyContent: "center", gap: 14, background: "var(--surface-2)" }}>
          <span className="w-[48px] h-[48px] rounded-[13px] grid place-items-center bg-surface shadow-[var(--sh-sm)] text-coral-deep"><Icons.sparkle size={24} /></span>
          <div>
            <h3 className="text-[18px] mb-[6px]">Something bigger in mind?</h3>
            <p className="text-[13.5px] text-ink-2 leading-[1.5] m-0">Off-sites, launches, away-days — our team curates a custom experience for any group size.</p>
          </div>
      ...
      <div className="corp-feature" style={{ marginTop: "calc(var(--gap) + 16px)" }}>
        <div className="relative max-w-[560px]">
          ...
          <h2 className="text-[#fff] leading-[1.06] mb-[12px]" style={{ fontSize: "clamp(24px,3vw,34px)" }}>Bring the whole team together.</h2>
          <p className="text-[rgba(255,255,255,.85)] text-[15px] leading-[1.6] mb-[20px]" style={{ maxWidth: "48ch" }}>
      ...
      <div className="mt-[40px] mx-0 mb-[14px]"><SectionHead eyebrow="Managed by your company" title="Your company plan" /></div>
      <div className="grid grid-cols-[1fr_1.25fr] gap-[var(--gap)] items-start">
        <div className="corp-card">
          <div className="flex items-center gap-[12px] mb-[18px]">
            ...
            <div><div className="font-extrabold font-display text-[16px]">{COMPANY.name}</div><div className="text-[12.5px] text-ink-3 font-semibold">{COMPANY.plan} · since {COMPANY.since}</div></div>
          </div>
          <div className="mb-[8px] flex justify-between text-[13px] font-bold"><span className="text-ink-2">Seats in use</span><span>{COMPANY.seatsUsed} / {COMPANY.seats}</span></div>
          <div className="h-[8px] rounded-[99px] bg-surface-2 overflow-hidden mb-[18px]"><div style={{ height: "100%", width: `${(COMPANY.seatsUsed / COMPANY.seats) * 100}%`, background: "linear-gradient(90deg,var(--coral),var(--orange))", borderRadius: 99 }} /></div>
        ...
        <div className="corp-card" style={{ padding: 0, overflow: "hidden" }}>
          ...
          {CORP_INVOICES.map((inv) => (
            <div key={inv.id} className="flex items-center gap-[14px] py-[14px] px-[22px] border-t border-line">
              <span className="w-[38px] h-[38px] rounded-[11px] flex-none grid place-items-center" style={{ background: inv.status === "paid" ? "color-mix(in srgb,#1FA46E 13%,transparent)" : "color-mix(in srgb,var(--orange) 14%,transparent)", color: inv.status === "paid" ? "#1FA46E" : "var(--orange-deep)" }}><Icons.wallet size={18} /></span>
              <div className="flex-1 min-w-0">
                <div className="font-bold text-[14px]">{inv.id}</div>
                <div className="text-[12.5px] text-ink-3 font-semibold">{inv.label} · {inv.date}</div>
              </div>
              <div className="text-right">
                <div className="font-display font-bold text-[14px]">{fmt(inv.amount)}</div>
                <span className="text-[11.5px] font-extrabold uppercase tracking-[.04em]" style={{ color: inv.status === "paid" ? "#1FA46E" : "var(--orange-deep)" }}>{inv.status}</span>
              </div>
    ...
    <div className="card" style={{ overflow: "hidden" }}>
      <div className="relative h-[120px]" style={{ background: e.gradient }}>
        ...
        <div className="absolute top-[12px] left-[12px] flex gap-[7px]">
          <span className="bg-[rgba(255,255,255,.92)] text-[#241C2E] py-[5px] px-[11px] rounded-[99px] text-[11.5px] font-extrabold">{CAL_WD[calDow(ev.day)]} {ev.day} Jun · {ev.time}</span>
        </div>
        <span className="absolute bottom-[12px] left-[14px] text-[#fff] font-display font-extrabold text-[18px]" style={{ textShadow: "0 1px 10px rgba(0,0,0,.4)" }}>{ev.title}</span>
      </div>
      <div className="py-[15px] px-[17px]">
        ...
        <div className="flex items-center gap-[10px]">
          <div className="av-stack">
            ...
            <span className="ml-[-9px] w-[28px] h-[28px] rounded-[99px] bg-surface-2 border-2 border-surface grid place-items-center text-[11px] font-extrabold text-ink-2">+{ev.extra}</span>
          </div>
          <span className="text-[12.5px] text-ink-3 font-bold">{ev.going.length + ev.extra}/{ev.cap} going</span>
...
function Lbl({ children }: { children: React.ReactNode }) { return <div className="text-[12.5px] font-bold text-ink-2 mb-[7px]">{children}</div>; }
    ...
    <Modal onClose={onClose} maxWidth={500}>
      {step === 0 ? (
        <div className="py-[26px] px-[28px]">
          ...
          <p className="text-ink-2 text-[14px] mt-0 mx-0 mb-[20px]">Our events team replies within one business day with a tailored plan.</p>
          <div className="flex flex-col gap-[16px]">
            <div><Lbl>Team size — <b className="text-coral-deep">{size} people</b></Lbl><input type="range" min={4} max={80} value={size} onChange={(e) => setSize(+e.target.value)} className="w-full" style={{ accentColor: "var(--coral)" }} /></div>
            <div><Lbl>Vibe</Lbl><div className="flex flex-wrap gap-[8px]">{MOOD_ORDER.map((k) => <MoodChip key={k} mood={k} active={vibe === k} onClick={() => setVibe(k)} />)}</div></div>
            <div className="flex gap-[12px]">
              <div className="flex-1"><Lbl>Preferred date</Lbl><Input type="date" value={date} onChange={(e) => setDate(e.target.value)} /></div>
              <div className="flex-1"><Lbl>Budget / person</Lbl><div className="cal-sel" style={{ display: "block" }}><Select value={budget} onChange={(e) => setBudget(e.target.value)} style={{ appearance: "none" }}><option value="low">Up to 2 000 ₽</option><option value="mid">2 000–5 000 ₽</option><option value="high">5 000 ₽+</option></Select></div></div>
            </div>
            <div><Lbl>Anything else?</Lbl><Textarea rows={3} placeholder="Dietary needs, accessibility, the occasion…" value={note} onChange={(e) => setNote(e.target.value)} style={{ resize: "vertical" }} /></div>
          </div>
        ...
        <div className="py-[34px] px-[30px] text-center">
          <div className="w-[66px] h-[66px] rounded-[99px] bg-[color-mix(in_srgb,#1FA46E_14%,transparent)] text-[#1FA46E] grid place-items-center mt-0 mx-auto mb-[18px]"><Icons.check size={34} /></div>
          <h2 className="text-[23px] mb-[8px]">Request sent ✦</h2>
          <p className="text-ink-2 text-[14.5px] mt-0 mx-auto mb-[22px] leading-[1.55]" style={{ maxWidth: "36ch" }}>Our events team will email you a tailored plan for <b>{size} people</b> within one business day.</p>
  ...
  return (
    <Modal onClose={onClose} maxWidth={460}>
      {!sent ? (
        <div className="py-[26px] px-[28px]">
          ...
          <div className="corp-gift" style={{ marginBottom: 20, cursor: "default" }}>
            <div className="relative flex items-center justify-between">
              <div><div className="text-[11.5px] tracking-[.1em] opacity-[.7] font-bold">JOYMAP GIFT</div><div className="font-display font-extrabold text-[30px] text-[#FFF3E4] mt-[4px]">{fmt(amt)}</div></div>
              <Icons.gift size={34} style={{ color: "var(--gold)" }} />
            </div>
          </div>
          <Lbl>Amount</Lbl>
          <div className="grid grid-cols-[repeat(4,1fr)] gap-[8px] mb-[16px]">
            {GIFT_DENOMS.map((d) => <div key={d} className={`gift-denom ${amt === d ? "on" : ""}`} onClick={() => setAmt(d)}><div className="font-display font-extrabold text-[15px]">{fmt(d)}</div></div>)}
          </div>
          <Lbl>Colleague&apos;s email</Lbl>
          <Input type="email" placeholder="name@acme.com" value={to} onChange={(e) => setTo(e.target.value)} style={{ marginBottom: 14 }} />
          <Lbl>Message (optional)</Lbl>
          <Textarea rows={2} placeholder="Thanks for everything — enjoy! 🎉" value={msg} onChange={(e) => setMsg(e.target.value)} style={{ resize: "vertical", marginBottom: 20 }} />
        ...
        <div className="py-[34px] px-[30px] text-center">
          <div className="w-[66px] h-[66px] rounded-[99px] bg-[color-mix(in_srgb,var(--gold)_18%,transparent)] text-[#B97714] grid place-items-center mt-0 mx-auto mb-[18px]"><Icons.gift size={32} /></div>
          <h2 className="text-[23px] mb-[8px]">Gift on its way 🎁</h2>
          <p className="text-ink-2 text-[14.5px] mt-0 mx-auto mb-[22px] leading-[1.55]" style={{ maxWidth: "34ch" }}>A <b>{fmt(amt)}</b> Joymap gift card is heading to {to || "your colleague"}.</p>
          <Btn size="md" onClick={onClose}>Done</Btn>
        </div>
      )}
    </Modal>
