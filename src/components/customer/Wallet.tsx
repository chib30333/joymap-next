  ...
  return (
    <div className="anim-fade max-w-[640px]">
      <div className="card" style={{ padding: 28, background: "linear-gradient(150deg,#5E1014,var(--maroon))", color: "#F3EBE0", border: "1px solid color-mix(in srgb,var(--red) 55%,transparent)", position: "relative", overflow: "hidden", marginBottom: 24 }}>
        <div className="text-[13.5px] opacity-[.82] font-semibold mb-[8px]">Joymap balance</div>
        <div className="font-display font-extrabold text-[40px] tracking-[-.02em]">{fmt(wallet)}</div>
      ...
      {tx.length === 0 ? (
        ...
        <div className="card" style={{ overflow: "hidden" }}>
          {tx.map((t, i) => (
            <div key={t.id} className="flex items-center justify-between pt-[15px] pb-[15px] pl-[20px] pr-[20px]" style={{ borderTop: i ? "1px solid var(--line)" : "none" }}>
              <div><div className="font-bold text-[14.5px]">{t.label}</div><div className="text-[12.5px] text-ink-3 font-semibold">{t.date}</div></div>
              <span className="font-extrabold font-display" style={{ color: t.amount > 0 ? "var(--m-calm)" : "var(--ink)" }}>{t.amount > 0 ? "+" : "−"}{fmt(Math.abs(t.amount))}</span>
            </div>
          ))}
        </div>
      )}
  ...
  return (
    <Modal onClose={onClose} maxWidth={400}>
      <div className="p-[26px]">
        ...
        <Input type="number" value={amt} onChange={(e) => setAmt(Math.max(+e.target.value || 0, 0))} style={{ marginBottom: 18 }} />
