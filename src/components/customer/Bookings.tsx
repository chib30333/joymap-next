    ...
    <div className="anim-fade">
      ...
      {list.length === 0 ? (
        <div className="text-center py-[70px] px-[20px] text-ink-3">
          <div className="w-[64px] h-[64px] rounded-[99px] bg-surface-2 grid place-items-center mt-0 mx-auto mb-[16px]"><Icons.calendar size={28} /></div>
          <h3 className="text-[19px] text-ink">{tab === "upcoming" ? "Nothing booked yet" : "No past experiences"}</h3>
          <p className="max-w-[360px] mt-[8px] mx-auto mb-0">{tab === "upcoming" ? "Find something in Discover and book your first experience — it will appear here." : "Completed and cancelled bookings will show up here."}</p>
        </div>
      ) : (
                ...
                <div className="flex-1 py-[18px] px-[22px] flex items-center gap-[20px] flex-wrap">
                  <div className="flex-1 min-w-[180px]">
                    ...
                    <div className="flex gap-[14px] mt-[8px] text-ink-3 text-[13.5px] font-semibold flex-wrap">
                      <span className="inline-flex gap-[5px] items-center"><Icons.calendar size={14} />{b.date}</span>
                      <span className="inline-flex gap-[5px] items-center"><Icons.clock size={14} />{b.time}</span>
                      <span className="inline-flex gap-[5px] items-center"><Icons.pin size={14} />{e.area}</span>
                    </div>
                  </div>
                  {tab === "upcoming" ? (
                    <div className="flex flex-col gap-[8px] items-end">
                      ...
                      <span className="text-[12px] text-ink-3 font-semibold">{b.code}</span>
                    </div>
                  ) : (
                    <div className="flex flex-col gap-[8px] items-end">
  ...
  return (
    <Modal onClose={onClose} maxWidth={360}>
      <div className="p-[28px] text-center">
        ...
        <div className="text-[13.5px] text-ink-3 font-semibold mb-[18px]">{b.date} · {b.time}</div>
        <div className="card" style={{ padding: 22, background: "var(--surface-2)" }}>
          <QR />
          <div className="mt-[14px] [font-family:var(--display)] font-extrabold tracking-[.12em] text-[18px]">{b.code}</div>
  ...
  return (
    <Modal onClose={onClose} maxWidth={420}>
      <div className="p-[26px]">
        ...
        <div className="text-[13.5px] text-ink-3 font-semibold mb-[18px]">{b.exp?.title ?? ""} · {b.date}</div>
        ...
        <Textarea rows={3} placeholder="Tell others what you loved (optional)…" value={text} onChange={(e) => setText(e.target.value)} style={{ resize: "vertical", marginBottom: 18 }} />
  ...
  return (
    <Modal onClose={onClose} maxWidth={440}>
      <div className="p-[26px]">
        ...
        <div className="text-[13.5px] text-ink-3 font-semibold mb-[18px]">{b.exp?.title ?? ""} — the provider will re-confirm.</div>
  ...
  return (
    <Modal onClose={onClose} maxWidth={400}>
      <div className="p-[26px] text-center">
        <div className="w-[56px] h-[56px] rounded-[99px] bg-coral-soft text-coral grid place-items-center mt-0 mx-auto mb-[14px]"><Icons.close size={26} /></div>
        <h3 className="text-[20px] mb-[8px]">Cancel this booking?</h3>
        <p className="text-ink-2 text-[14px] mb-[6px]"><b>{b.exp?.title ?? ""}</b> · {b.date}, {b.time}</p>
        <p className="text-ink-3 text-[13px] mb-[20px]">{b.pay === "wallet" ? `${fmt(b.total)} will be refunded to your Joymap balance.` : "Free cancellation up to 12h before the start."}</p>
        <div className="flex gap-[10px]">
